...
export default {
    ...
    computed: {
        import(){
            return this.prod.units * this.prod.price;
        }
    },
    methods:{
        addUnit() {
            var id = this.prod.id;
            var plusUnits = ++this.prod.units;
            store.plusUnitProducts(id, plusUnits);
          },
        quitUnit() {
            var id = this.prod.id;
            var plusUnits = --this.prod.units;
            store.minusUnitProducts(id, plusUnits);
        },
        quitProd(){
            var prod = this.prod;
            store.delProducts(prod);
        }
    }
   }
</script>
